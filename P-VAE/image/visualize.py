...
class Visualizer(object):
    ...
    def plot_tests(self, images, bbox=None, filename=None):
        rows, cols = 8, 16
        ...
        for i, image in enumerate(images):
            plt.imshow(image)
            plt.axis('off')
            plt.savefig(str(filename)+ ("%s"%(i)) + ".png", bbox_inches='tight',pad_inches = 0 )
    ...
    def plot(self, epoch, x, mask, bbox, x_recon, x_gen):
        if self.at_start:
            self.plot_tests(x * mask + .5 * (1 - mask), bbox,
                              self.recons_dir / f'groundtruth.png')
            self.at_start = False

        self.plot_tests(x * mask + x_recon * (1 - mask), bbox,
