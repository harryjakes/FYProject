...
def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=8):
    ...
    m1, s1 = compute_statistics_of_path('FIDpaths/a/', model, batch_size,
                                        dims, device, num_workers)
    m2, s2 = compute_statistics_of_path('FIDpaths/b/', model, batch_size,
                                        dims, device, num_workers)
...
def main():
    ...
    device = torch.device('cuda' if (torch.cuda.is_available()) else 'cpu')
    fid_value = calculate_fid_given_paths(None,
                                          args.batch_size,
                                          device,
                                          args.dims,
                                          args.num_workers)
