...
class MaskedImage(datasets.ImageFolder):
    def __init__(self, data_dir='dataset', image_size=64, random_seed=0):
...
class BlockMaskedImage(MaskedImage):
    def __init__(self, block_len=None, *args, **kwargs):
        self.block_len = block_len
        super().__init__(*args, **kwargs)

    def generate_masks(self):
        d0_len = d1_len = self.image_size #d0_len = d1_len = self.image_size
        d0_min_len = 12
        ...
        for i in range(n_masks):
            if self.block_len is None:
                ...
                d0_mask_len = self.image_size
                d1_mask_len = self.block_len
...
class IndepMaskedImage(MaskedImage):
